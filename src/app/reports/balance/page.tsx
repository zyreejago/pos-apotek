...
export default function BalanceSheetAccountingPage() {
  ...
  const fetchData = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetch(`http://localhost:5000/api/reports/balance-accounting?month=${month}&year=${year}`, {
                    ...
                    {assetAccounts.map((a) => (
                      <tr key={a.id} className="hover:bg-gray-50 transition-colors">
                        ...
                        <td className="px-6 py-3 text-right text-gray-600">
                          {a.normal_balance === 'debit' && getAccountBalance(a) > 0 ? formatCurrency(getAccountBalance(a)) : ''}
                        </td>
                        <td className="px-6 py-3 text-right text-gray-600">
                          {a.normal_balance === 'kredit' && getAccountBalance(a) > 0 ? formatCurrency(getAccountBalance(a)) : ''}
                        </td>
                      </tr>
                    ))}
