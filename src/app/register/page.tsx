  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        goeyToast.success('Registrasi berhasil!');
        router.push('/dashboard');
      } else {
        const message = data.message || 'Registrasi gagal';
        setError(message);
        goeyToast.error('Registrasi Gagal');
      }
    } catch {
        ...
        <form onSubmit={handleRegister} className="space-y-4">
          ...
          <div className="flex items-start gap-2">
            ...
            <label htmlFor="terms" className="text-xs text-gray-600">
              I accept <a href="#" className="text-blue-500 font-medium">Terms & Conditions</a>
            </label>
          </div>
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full bg-blue-500 text-white py-2.5 rounded-md font-semibold hover:bg-blue-600 transition-colors disabled:opacity-70 disabled:cursor-not-allowed shadow-sm text-xs"
          >
