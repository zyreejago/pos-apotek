...
export default function Sidebar() {
  ...
  return (
    <div className="w-64 h-screen bg-white border-r border-gray-200 flex flex-col font-sans shrink-0 sticky top-0">
      {/* Header */}
      <div className="p-6 flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xl font-bold text-slate-800">Sales Point</span>
        </div>
        <button className="p-1 rounded hover:bg-gray-100 border border-gray-200 text-gray-400">
             <ChevronLeft size={16} />
        </button>
      </div>
      ...
      <div className="flex-1 overflow-y-auto py-2 px-3 space-y-1 custom-scrollbar">
        ...
        <NavItem href="/dashboard" icon={<LayoutDashboard size={20} />} label="Dashboards" active={isActive('/dashboard') || isActive('/')} />
        <NavItem href="/outlets" icon={<Store size={20} />} label="Outlets" active={isActive('/outlets')} />
        <NavItem href="/products" icon={<Package size={20} />} label="Products" active={isActive('/products')} />
        <NavItem href="/stock-management" icon={<ClipboardList size={20} />} label="Management Stock" active={isActive('/stock-management')} />
        <NavItem href="/stock-opname" icon={<ClipboardCheck size={20} />} label="Stock Opname" active={isActive('/stock-opname')} />
        <NavItem href="/suppliers" icon={<Users size={20} />} label="Suppliers" active={isActive('/suppliers')} />
        <NavItem href="/transactions" icon={<ShoppingCart size={20} />} label="Transactions" active={isActive('/transactions')} />
        <NavItem href="/cashiers" icon={<UserCheck size={20} />} label="Management Cashier" active={isActive('/cashiers')} />
        
        {/* Expandable Menu */}
        <div>
            <button 
                onClick={() => setIsReportOpen(!isReportOpen)}
                className={`w-full flex items-center justify-between px-3 py-2.5 rounded-lg transition-colors group ${pathname.startsWith('/reports') ? 'bg-gray-50' : 'hover:bg-gray-50'}`}
            >
                <div className="flex items-center gap-3">
                    <span className={`group-hover:text-slate-600 ${pathname.startsWith('/reports') ? 'text-slate-600' : 'text-slate-400'}`}>
                        <FileText size={20} />
                    </span>
                    <span className={`font-medium ${pathname.startsWith('/reports') ? 'text-slate-800' : 'text-slate-600'}`}>Sales Report</span>
                </div>
                <span className="text-slate-400 text-lg leading-none">{isReportOpen ? '−' : '+'}</span>
            </button>
        ...
        <NavItem href="/recommendations" icon={<Rocket size={20} />} label="Recommendation Stock" active={isActive('/recommendations')} />
        <NavItem href="/substitutions" icon={<Repeat size={20} />} label="Subtitusi Products" active={isActive('/substitutions')} />
...
function NavItem({ icon, label, href, active = false }: { icon: React.ReactNode, label: string, href: string, active?: boolean }) {
    return (
        <Link href={href} className={`flex items-center gap-3 px-3 py-2.5 rounded-lg transition-colors ${active ? 'bg-blue-50 text-blue-600' : 'text-slate-600 hover:bg-gray-50 group'}`}>
