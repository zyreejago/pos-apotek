...
const stockForecast = require('./routes/stock-forecast');
...
async function main() {
  ...
  console.log('\n=== 2. Load data dari Excel dengan loadMonthlySalesFromXlsx dari stock-forecast.js ===');
  ...
  function loadMonthlySalesFromXlsx(filePath) {
    ...
    if (!fs.existsSync(filePath)) return { salesByName: new Map(), endDate: null };
  ...
  function buildDailySeriesFromDayMap(dayMap, endDate, days) {
    console.log('=== buildDailySeriesFromDayMap (stock-forecast.js) ===');
  ...
  if (salesByName.has(norm)) {
    ...
    const seriesDays = 365;
    const seriesEnd = endDate;
    const series = buildDailySeriesFromDayMap(dayMap, seriesEnd, seriesDays);
