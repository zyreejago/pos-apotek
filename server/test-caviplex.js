...
function imputeMissingValues(series) {
  console.log('\n=== imputeMissingValues ===');
  console.log('Input series:', series.slice(0, 20));
  ...
  for (let i = 0; i < result.length; i++) {
    if (result[i] === null || result[i] === undefined || !Number.isFinite(result[i])) {
      ...
      console.log(`i=${i}: prevValid=${prevValid}, nextValid=${nextValid}`);
      ...
      if (prevValid !== undefined && nextValid !== null) {
        ...
        console.log(`  → Menjadi 0`);
        result[i] = 0;
      }
    }
  }
  
  console.log('Output series:', result.slice(0, 20));
  return result;
}
...
function buildDailySeriesFromDayMap(dayMap, startDate, endDate) {
  console.log('=== buildDailySeriesFromDayMap ===');
...
async function main() {
  const productId = 61; // ID produk Caviplex Tab dari database (dari test-all-products.js)
  ...
  if (salesByName.has(norm)) {
    ...
    const series = buildDailySeriesFromDayMap(dayMap, startDate, endDate);
    ...
    console.log('\nSemua nama produk di Excel:');
    Array.from(salesByName.keys()).slice(0, 20).forEach(name => console.log('-', name));
  }
}
