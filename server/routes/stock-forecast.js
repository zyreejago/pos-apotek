...
function createSlidingWindow(series, windowSize = 7, forecastHorizon = 7) {
  ...
  const dataset = [];
  for (let i = 0; i + w + fh <= series.length; i += 1) {
    const input = series.slice(i, i + w);
    const output = series.slice(i + w, i + w + fh).reduce((a, b) => a + (Number.isFinite(b) ? b : 0), 0);
...
async function callGeminiAPI(promptJson, instruction) {
  const apiKeys = [
  ...
  let lastError = null;
  for (const keyIndex in apiKeys) {
    const key = apiKeys[keyIndex];
    const keyNum = Number(keyIndex) + 1;
    console.log(`\n========== TRYING API KEY ${keyNum}/${apiKeys.length} ==========`);
      ...
      try {
        const body = {
          contents: [
            {
              role: 'user',
              parts: [{ text: 'Kamu adalah asisten AI yang membantu peramalan stok obat di apotek. Jawaban kamu HANYA JSON saja, tanpa kata-kata lain!' }],
            },
            {
              role: 'model',
              parts: [{ text: 'Baik, saya hanya akan mengeluarkan JSON tanpa kata-kata lain.' }],
            },
            {
              role: 'user',
        ...
        const aiText = Array.isArray(parts) && parts.length > 0 
        ...
        return { instruction, aiText, model, usage: data?.usageMetadata };
      } catch (e) {
        console.error('\n❌ GEMINI ERROR!');
        console.error(`API Key ${keyNum}, Model ${model}`);
  ...
  err.code = lastError?.code || 'gemini_error';
  throw err;
}
...
function fallbackExponentialSmoothing(series, alpha = 0.6) {
...
async function generateStockRecommendation(options) {
  ...
  try {
    ...
    const instruction =
      ...
      '3. Jika punya_satuan_besar = true:\n' +
      'tambahan_stok harus habis dibagi jumlah_per_satuan_besar.\n\n' +
      'ANTI-HALUSINASI:\n' +
    ...
    const geminiResult = await callGeminiAPI(promptJson, instruction);
    ...
    method = 'fallback';
    validationReason = e?.code || 'gemini_error';
    perkiraanPerHari = perDayBaseline;
  }

  if (method === 'fallback') {
    if (totalSold === 0 && validationReason === 'gemini_error') {
      validationReason = 'no_sales_data';
    }
  ...
  console.log('Prompt Gemini:', geminiPrompt);
  console.log('Response Gemini:', geminiResponse);
...
function loadMonthlySalesFromXlsx(filePath) {
  ...
  for (const sheetName of wb.SheetNames) {
    ...
    for (let c = 0; c < header.length; c += 1) {
      ...
      if (typeof v === 'string') {
        ...
        // Match both YYYY-MM-DD and M/D/YY formats
        if (/^\d{4}-\d{2}-\d{2}$/.test(trimmed) || /^\d{1,2}\/\d{1,2}\/\d{2}$/.test(trimmed)) {
          // Normalize M/D/YY to YYYY-MM-DD
          let normalizedDate = trimmed;
          const mdyMatch = trimmed.match(/^(\d{1,2})\/(\d{1,2})\/(\d{2})$/);
          if (mdyMatch) {
            const month = mdyMatch[1].padStart(2, '0');
            const day = mdyMatch[2].padStart(2, '0');
            const year = '20' + mdyMatch[3];
            normalizedDate = `${year}-${month}-${day}`;
          }
          dateCols.push({ c, date: normalizedDate });
  ...
  if (allDates.length > 0) {
    ...
    finalStart = new Date(Math.min(...dates));
    finalEnd = new Date(Math.max(...dates));
  }

  return { salesByName, startDate: finalStart, endDate: finalEnd };
}
...
module.exports = function registerStockForecastRoutes(app, pool, authenticate, checkPermission) {
  app.post(
    '/api/forecast/run',
    authenticate,
    async (req, res) => {
      try {
        console.log('Starting manual forecast job with Gemini...');
  ...
  app.get(
    ...
    async (req, res) => {
      ...
      try {
        const params = [];
        let sql = 'SELECT id, name, stock, unit FROM products';
        if (search) {
          sql += ' WHERE name LIKE ?';
          params.push(`%${search}%`);
        }
...
module.exports.imputeMissingValues = imputeMissingValues;
module.exports.normalizeAnomalies = normalizeAnomalies;
