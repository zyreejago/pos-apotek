...
async function generateStockRecommendation(options) {
  ...
  //const datasetWindowed = (createSlidingWindow(options.geminiSeries || series, windowSize, 1) || []).map(d => ({ input: d.input }));
  ...
  const processProduct = async (p) => {
    ...
    if (allRows.length > 0) {
      ...
      if (dates.length > 0) {
        const minDate = new Date(Math.min(...dates));
        const maxDate = new Date(Math.max(...dates));
        actualDataDays = Math.round((maxDate - minDate) / (1000 * 60 * 60 * 24)) + 1;
      }
    }
